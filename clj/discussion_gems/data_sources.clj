...
(defn comments-all-rdd
  ...
  (->>
    ...
    (parsing/backfill-reddit-name "t1_")))
...
(defn submissions-all-rdd
  ...
  (->>
    ...
    (parsing/backfill-reddit-name "t3_")))
    ...
    (uspark/run-local
      (fn [sc]
        (->> (comments-all-rdd sc)
          ...
          (uspark/save-to-hadoop-text+fressian-seqfile "../derived-data/reddit-france/comments/RC-enriched_v2_sample1e-2.seqfile")))))
    ...
    (uspark/run-local
      (fn [sc]
        (->> (submissions-all-rdd sc)
          ...
          (uspark/save-to-hadoop-text+fressian-seqfile "../derived-data/reddit-france/submissions/RS_sample1e-2.seqfile")))))
